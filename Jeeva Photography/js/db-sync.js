...
const JeevaDB = (() => {
  ...
  function getVideos() {
    ...
    if (custom && Array.isArray(custom) && custom.length) {
      // If old dataset (more than 3 videos or old mp4s) is stored in localStorage, refresh with exact 3 YouTube videos
      const needsSync = custom.length > 3 || custom.some(v => v.id === 'video-4' || v.id === 'video-5' || (v.videoSrc && (v.videoSrc.startsWith('videos/') || v.videoSrc.includes('.mp4'))));
      if (needsSync && typeof VIDEO_HIGHLIGHTS_DATA !== 'undefined') {
        saveVideos(JSON.parse(JSON.stringify(VIDEO_HIGHLIGHTS_DATA)));
